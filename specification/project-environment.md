...
| **Product** | [`product/`](product/) | Feature acceptance criteria (`01-login` … `06-customer-bookings`) |
...
| **Decisions** | [`decisions/`](decisions/) | Stubs → repo-root [`adr/`](../adr/) |
...
[ADR-0005](../adr/0005-mockoon-echoes-return-notes.md).
...
Auth and booking routes are defined in OpenAPI and served by the generated Mockoon env. Product behaviour: [product/01-login.md](product/01-login.md). Mock auth is canned (no real credential verification) — see [api/README.md](api/README.md) and [ADR-0004](../adr/0004-three-layer-mock-strategy.md).
...
## GitHub Flow (CI / Release)

Normative OpenSpec: [`openspec/specs/ci-pipelines/spec.md`](../openspec/specs/ci-pipelines/spec.md). ADR-0002.

| Pipeline | Caller | Trigger | Stages |
|----------|--------|---------|--------|
| Fast Feedback | `mobile-fast-feedback-caller.yml` | Push to branches other than `master`/`develop` | Integration only |
| CI | `mobile-ci-caller.yml` | PR / push to `develop` | Integration (may reuse Fast Feedback), QC, Security, CodeQL, Mock contract tests |
| Release | `mobile-release-caller.yml` | Manual `workflow_dispatch` | Integration, QC, unsigned APK, DAST, GitHub Release |

Reusable pipelines use JDK 17, compile SDK 35, Node 22 for mock tests, and `DEFAULT_APP_REPOSITORY=Heavy-Rental/heavy-rental-mobile`. This repo has **no** Academy/paid CD. `.github/workflows/android-ci.yml` is a legacy leftover (HR-157), not GitHub Flow SoT.

## Related specs

- [00-project-overview.md](00-project-overview.md) — project purpose, scope, stack
- [setup-guide.md](setup-guide.md) — first-time clone and run
- [specification/README.md](README.md) — SDD overview
- [specification/api/README.md](api/README.md) — API + mock commands
- [ADR-0004](../adr/0004-three-layer-mock-strategy.md) — why three mock layers
- [ADR-0007](../adr/0007-default-http-target-is-spring-boot.md) — default `:8080`
